    ...
    pub fn parse_action() -> Result<(Self, CommandAction), clap::Error> {
        ...
        if let Some(opt) = &cli.print {
            if !cli.note.is_empty() {
                return Err(clap::Error::raw(ErrorKind::ArgumentConflict, "cannot mix note text with --print/-p"));
            }
            let count = opt.unwrap_or(10);
            return Ok((cli, CommandAction::Print { count }));
        }
        if cli.note.is_empty() {
            return Err(clap::Error::raw(ErrorKind::MissingRequiredArgument, "supply note text or use --print"));
        }
        let text = cli.note.join(" ").trim().to_string();
        if text.is_empty() {
            return Err(clap::Error::raw(ErrorKind::InvalidValue, "note text cannot be empty"));
        }

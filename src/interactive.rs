...
pub fn run_interactive_session<C: Clock, R: BufRead, W: Write>(
    ...
    loop {
        ...
        if line_buf.trim().is_empty() {
            continue; // ignore blank/whitespace lines
        }
        // Preserve trailing spaces: we only trimmed for emptiness check above.
        // Append note line
        append_note_line_to_file_with_clock(note_file_path, clock, datetime_pattern, &line_buf)?;
        count += 1;
    }
    if count == 0 { Ok(InteractiveOutcome::Empty) } else { Ok(InteractiveOutcome::Added(count)) }
}

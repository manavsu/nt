...
fn append_from_stdin_multiline_creates_two_separate_notes() {
    ...
    let first_matches: Vec<&str> = lines
        .iter()
        .copied()
        .filter(|l| l.contains("first line"))
        .collect();
    let second_matches: Vec<&str> = lines
        .iter()
        .copied()
        .filter(|l| l.contains("second line"))
        .collect();
    assert_eq!(
        first_matches.len(),
        1,
        "expected one line containing first line: {stdout}"
    );
    assert_eq!(
        second_matches.len(),
        1,
        "expected one line containing second line: {stdout}"
    );
    assert_ne!(
        first_matches[0], second_matches[0],
        "expected different lines for each note"
    );
}
...
fn append_from_stdin_reports_added_count() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(
        stdout.contains("added 3 notes"),
        "stdout missing plural summary: {stdout}"
    );
